...
async def get_price(min_price: Optional[float] = Query(ge = 0.0), max_price: Optional[float] = Query(le = 1000.0)):
  filtered_prices = [
    price 
...
@app.get("/tasks")
async def get_tasks(task_db: str):
  return {"tasks": tasks_db}

@app.post("/tasks")
async def add_task(task_name: str = Query(None, min_length = 3, max_length = 50)):
   tasks_db.append(task_name)
   return {"message": "Task added successfully", "task": task_name}

@app.delete("/tasks/{task_index}")
async def delete_task(task_index: int = Path(..., ge = 0)) -> Dict[str, object]:
